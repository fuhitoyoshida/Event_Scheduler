...
public class Scheduler {
	...
	private static boolean initializeFromInputFile(String resourceListFile) {
		 ...
		 */
		File file = new File(resourceListFile);
		try {
			...
			while(scanner.hasNextLine()){
				...
				long start = convertToTime(scanner.nextLine());
				long end = convertToTime(scanner.nextLine());
				String org = scanner.nextLine();
				String description = scanner.nextLine();

				schedulerDB.addEvent(resource, start, end, name, org, description);
			}
			scanner.close();
			return true;
